...
interface KurikulumPaketBTabProps {
...
export const KurikulumPaketBTab: React.FC<KurikulumPaketBTabProps> = ({
  programDetail,
  strukturMapel,
  currentUser,
  onEditProgram,
  ...
  const mapelPaketB = strukturMapel.filter(item => item.program === 'PAKET B');
  const totalSkk = mapelPaketB.reduce((acc, curr) => acc + (curr.alokasi_skk || 0), 0);

  const umumList = mapelPaketB.filter(m => m.kelompok === 'UMUM');
  const khususList = mapelPaketB.filter(m => m.kelompok === 'PEMBERDAYAAN' || m.kelompok === 'KETERAMPILAN');
  ...
  return (
    <div className="space-y-8 animate-fadeIn text-[#1F2937]">
          ...
          <div className="flex flex-col sm:flex-row md:flex-col gap-3 shrink-0">
            ...
            {isAdmin && onEditProgram && (
              ...
              >
                ...
                <span>Edit Program Paket B</span>
              </button>
            )}
          </div>
        </div>
      ...
      {/* Tingkatan Kelas & Karakteristik Belajar */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-gray-200 shadow-xs p-6 space-y-4">
          <h3 className="text-base font-bold text-[#172033] flex items-center gap-2 border-b border-gray-100 pb-3">
            ...
            <span>Tingkatan & Derajat Kompetensi</span>
          </h3>
          <div className="space-y-2.5 pt-1">
            {programDetail.tingkatan_kelas.map((tk, idx) => (
              <div key={idx} className="p-3.5 rounded-lg bg-[#F5F6F8] border border-gray-200 flex items-center gap-3">
                <span className="w-6 h-6 rounded-md bg-[#172033] text-white text-xs font-bold flex items-center justify-center shrink-0">
                  {idx + 1}
                </span>
                <span className="text-xs sm:text-sm font-semibold text-[#172033]">{tk}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-xs p-6 space-y-4">
          <h3 className="text-base font-bold text-[#172033] flex items-center gap-2 border-b border-gray-100 pb-3">
            ...
            <span>Strategi Pembelajaran & Vokasi</span>
          </h3>
          <div className="space-y-3 text-xs sm:text-sm text-[#1F2937] leading-relaxed pt-1">
            <div className="p-3.5 rounded-lg bg-[#F5F6F8] border border-gray-200">
              <span className="font-bold text-[#172033] block mb-1">Layanan Belajar:</span>
              <p className="text-gray-700">{programDetail.kelompok_belajar}</p>
            </div>
            <div className="p-3.5 rounded-lg bg-[#F5F6F8] border border-gray-200">
              <span className="font-bold text-[#172033] block mb-1">Pendekatan Andragogi:</span>
              <p className="text-gray-700">{programDetail.strategi_pembelajaran}</p>
            </div>
          </div>
        </div>
      </div>

      {/* Mata Pelajaran Paket B Overview (Clean Loose Table) */}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-gray-200 pb-4">
          <div>
            <h3 className="text-lg font-bold text-[#172033] flex items-center gap-2">
              ...
              <span>Daftar Mata Pelajaran Program Paket B</span>
            </h3>
            <p className="text-xs text-gray-500 mt-1">
              Sebanyak {mapelPaketB.length} mata pelajaran terbagi ke dalam Kelompok Umum dan Khusus Keterampilan.
            </p>
          </div>
          <button
